...
$(document).ready(function(){
		...
		$("#zzsc").find(".next").click(function(){
			//首先判断展示区域是否处于动画
			if(!$show.is(":animated")){
				$show.animate({left:'-1000px'/*+$width_box*/},"normal");
				page++;
				nav();
				$number=page-1;
		...
		if(!$show.is(":animated")){
			$show.animate({left:'0px'/*left:'+='+$width_box*/},"normal");
			page--;
			nav();
			$number=page-1;
			$("#zzsc").find(".nav a:eq("+$number+")").addClass("now").siblings("a").removeClass("now");
		...
		$("#zzsc").find(".nav a").click(function(){
				...
				page=$index+1;
				nav();
				$show.animate({left:'-1000px'/*left:-($width_box*$index)*/},"normal");	
				$(this).addClass("now").siblings("a").removeClass("now");
				return false;
		})
		$("#zzsc").find(".nav .now").click(function(){
				$index=$(this).index();
				page=$index+1;
				nav();
				$show.animate({left:'0px'/*left:-($width_box*$index)*/},"normal");	
				$(this).addClass("now").siblings("a").removeClass("now");
				return false;
		})
